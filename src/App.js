...
function App() {
  ...
  const handleCuisineClick = (cuisineData) => {
    setSelectedCuisine(cuisineData);
  };

  const filteredRestaurants = selectedCuisine
    ? restaurantsData.filter((restaurant) => restaurant.cuisine_id === selectedCuisine)
    : restaurantsData;

  return (
    <div>
      <h1 style={{textAlign:"center"}}>List of Cuisines:</h1>
      <div style={{display:"flex" , justifyContent:"center" ,margin:"2rem"}}>
        {cuisineData.map((cuisine) => (
          <button key={cuisine?.id} onClick={() => handleCuisineClick(cuisine.id)} >
            {cuisine?.name}
          </button>
        ))}
      </div>
      ...
      <div>
        {filteredRestaurants?.map((restaurant) => (
          <div key={restaurant?.id}>
            <h3 style={{textAlign:"center"}} >{restaurant?.name}</h3>
            <div style={{display : "flex ", flexDirection:"row", flexWrap:"wrap" ,justifyContent:"space-evenly" , padding:"o.2rem", margin :"0.5rem" }}>
            <RestaurentCard restaurent={filteredRestaurants}/>
            </div>
